...
def main():
    ...
    csvs['house/house_members.csv'] = [parse_member(m)
                                       for m in house_data['MemberData']['members']['member']]
    csvs['house/house_coms.csv'] = [parse_coms(c)
                                    for c in house_data['MemberData']['committees']['committee']]
    csvs['senate/metadata.csv'].append(parse_metadata(sen_data, 'senate'))
    csvs['senate/senators.csv'] = [parse_senator(s) for s in sen_data['senators']['senator']]
